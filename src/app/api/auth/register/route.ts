...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send verification email (optional - user can verify later from settings)
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        // Don't fail registration if email doesn't work - just log it
      }
    }

    return NextResponse.json(
      { message: "Registration successful. You can now login.", userId: user.id },
      { status: 201 }
    );
  } catch (error) {
